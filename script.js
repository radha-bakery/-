...
const UPI_ID = "paytm.s24oec1@pty";
const UPI_NAME = "Paytm";
...
document.getElementById("upiPayBtn").onclick = function(){
  ...
  const upiLink = `upi://pay?pa=${encodeURIComponent(UPI_ID)}&pn=${encodeURIComponent(UPI_NAME)}&am=${total}&cu=INR&tn=${encodeURIComponent("Radha Bakers Advance Booking")}`;
...
`Advance Payment: Paid / Pending%0A` +
`UPI ID: ${encodeURIComponent(UPI_ID)}%0A` +
`Payment Screenshot: Customer will send on WhatsApp%0A` +
